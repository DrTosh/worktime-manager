...
export class MonthController {
  ...
  calcTimeSum(month: Month, wage: number) {
    ...
    month.actualmoney =
      Math.round(
        this.timeController.toNumber(month.actualworktime) * wage * 100
      ) / 100;
    month.targetmoney =
      Math.round(
        this.timeController.toNumber(month.targetworktime) * wage * 100
      ) / 100;
  }
  ...
  public createWeeks(month: Month) {
    if (month.weeks.length == 0) {
      ...
      while (date.getMonth() == month.month) {
        ...
        // 0 == Sunday
        if (date.getDay() == 0 && nextDate.getDate() != 1) {
          week = new Week();
          month.weeks.push(week);
        }
